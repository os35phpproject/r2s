...
<ul class="nav nav-tabs">
  <li role="presentation" class="active"><a href=http://localhost/client_page.php>Home</a></li>
  <li role="presentation"><a href=http://localhost/myOrders.php>My Orders</a></li>
</ul>
</div>
...
echo "</img>"."<br/>";
echo  "<div id='namediv' class='name'>Hello,".$usrnm."</div><br/><br/><div class='name'><a href=http://localhost/logout.php>LogOut</a></div>";
echo "</div>";
echo "</div>";
...
<div id="latest_orders">
	...
	$db=mysqli_connect('localhost','root', 'root', 'phpproject');
	$query = "select max(ordate) from orders where orusid = ".$_SESSION['usrid'];
	$result = mysqli_query($db, $query);
	$themaxdate=mysqli_fetch_assoc($result);
	//echo $themaxdate["max(ordate)"];
		$obj->setTable("orders");
		$usrdata=$obj->select("all",["orusid"=>$_SESSION['usrid'],"ordate"=>$themaxdate["max(ordate)"],]);
		for($j=0;$j<mysqli_num_rows($usrdata);$j++){
 			$therec=mysqli_fetch_assoc($usrdata);
 			$obj->setTable("product");
 			$proinf=$obj->select("all",["id"=>$therec['orprid'],"statues"=>1,]);
 			for($h=0;$h<mysqli_num_rows($proinf);$h++){
 				$infofpro=mysqli_fetch_assoc($proinf);
 				//echo $infofpro['prname'];
 				if($h<mysqli_num_rows($proinf)-1){
 				?>
 				<div style="float:left;margin-right: 20px;">
 				<?php }else if($h==mysqli_num_rows($proinf)-1){ ?>
 				<div style="display:inline-block;margin-right: 20px;">
 				<?php } ?>
 					<img src=<?php echo "/".$infofpro['primage']; ?> width="70px" height="70px" id=<?php echo $infofpro['prname'];?> onclick=<?php echo "addproduct(".$infofpro['prname'].")"?> ></img><br/>
 					<?php echo $infofpro['prname']."<br/>".$infofpro['prprice']."LE" ?>
 				</div>
...
// console.log(notes);
theclient=document.getElementById("namediv").innerHTML;
theclient=theclient.split(",")[1];
...
//console.log("hamada");
	...
	if(theajax.readyState==4 && theajax.status==200){
		thereso=theajax.responseText;
			...
			document.getElementById("totalprice").innerHTML="<a href=http://localhost/client_page.php/?id="+theusrid+">send another order</a>";
